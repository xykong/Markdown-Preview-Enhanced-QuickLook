...
### Fixed
- **窗口最小尺寸限制**: 修复窗口无法缩小到 800×600 以下的问题 (#24)
...
### Fixed
- **多窗口 PDF 导出**: 修复同时打开多个 Markdown 文件时，Cmd+Shift+P 会弹出多个保存面板的问题（现在只导出当前活跃窗口）(#18)
- **PDF 导出内容分页**: 使用 NSPrintOperation 替换手动分页方案，PDF 内容不再出现截断或错位
- **PDF 导出字体大小**: 修复导出的 PDF 字体大小与应用预览不一致的问题
